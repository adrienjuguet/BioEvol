...
void World::evolution_step() {
  ...
  for (int i = 0; i < width_*height_; i++) {
      if (grid_cell_[i]->organism_ != nullptr) {
        ...
        max_fitness_ = grid_cell_[i]->organism_->fitness_ >
                       max_fitness_ ? grid_cell_[i]->organism_->fitness_
                                    : max_fitness_;
        min_fitness_ = grid_cell_[i]->organism_->fitness_ <
                       min_fitness_ ? grid_cell_[i]->organism_->fitness_
                                    : min_fitness_;
      }
  }
  ...
  for (int i = 0; i < width_*height_; i++) {
      if (grid_cell_[i]->organism_ == nullptr) {
        ...
        for (int x = i - Common::Duplicate_Neighbors_Offset; x <= i + Common::Duplicate_Neighbors_Offset; x++) {
            if (x >= 0 && x < width_*height_)
            {
                if (grid_cell_[x]->organism_ != nullptr) {
                  if (org_n != nullptr)
                    org_n = grid_cell_[x]->organism_->fitness_ <
                          org_n->fitness_ ? grid_cell_[x]->organism_
                                          : org_n;
                  else org_n = grid_cell_[x]->organism_;
                }
             }
          }
        ...
        if (org_n != nullptr) {
          ...
          grid_cell_[i]->organism_ = new Organism(new DNA(org_n->dna_));
          grid_cell_[i]->organism_->gridcell_ = grid_cell_[i];
          grid_cell_[i]->organism_->mutate();
